use windows::{
    Win32::{
        Foundation::E_FAIL,
        System::Performance::{
            PDH_COUNTER_INFO_W, PDH_HCOUNTER, PDH_HQUERY, PDH_MORE_DATA, PdhAddCounterW,
            PdhAddEnglishCounterW, PdhCloseQuery, PdhExpandWildCardPathW, PdhGetCounterInfoW,
            PdhOpenQueryW,
        },
    },
...
pub struct PerfQueryHandle(pub PDH_HQUERY);
...
pub fn add_perf_counters(
    ...
    let counter_handles: Vec<PDH_HCOUNTER> = unsafe {
        ...
        let result = PdhGetCounterInfoW(counter_handle, false, &mut buffer_size, None);
        ...
        let mut buffer_size = 0;
        let result = PdhExpandWildCardPathW(None, &full_path, None, &mut buffer_size, 0);
